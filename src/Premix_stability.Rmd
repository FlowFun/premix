...
The following functions are designed to transform and visualize the data. df_format reshapes the dataframe for analysis, and markers_violin creates violin plots to visualize the distribution of marker values across different days.
...
markers_violin <- function(df, marker_name, title = "Violin Plot") {
  ...
  df_formatted <- df_format(df) # Assuming df_format() reshapes the data appropriately for ggplot
  p <- ggplot(data = df_formatted, aes(x = day, y = value)) +
    geom_violin(trim = FALSE, alpha = 0.2, aes(fill = day)) +
    geom_boxplot(alpha = 0.2, show.legend = FALSE, width = 0.3) +
    geom_line(linetype = "dashed", linewidth = .6, aes(group = id), color = "gray20") +
    geom_point(aes(colour = day), alpha = 1, size = 2) +
    xlab("Day") + 
    ylab(marker_name) +
    ggtitle(title) + 
    theme_bw() +
